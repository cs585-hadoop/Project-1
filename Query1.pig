cus = LOAD 'hdfs://localhost:8020/user/hadoop/customer/customer.csv' USING PigStorage(',') as (ID,name,age,cc,salary);
trans = LOAD 'hdfs://localhost:8020/user/hadoop/transaction/transaction.csv' USING PigStorage(',') as (transID,cusID,transTotal,transNum,transDesc);
